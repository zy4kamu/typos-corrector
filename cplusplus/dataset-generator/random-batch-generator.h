...
public:
    ...
    void generate_country_state_dataset(const std::string& output_folder, size_t train_size, size_t test_size);
    void next(std::string& clean, std::string& contaminated);
private:
    void generate_country_state_dataset(const std::string& output_file, size_t size,
                                  std::map<std::string, size_t>& country_state_to_index);
    std::vector<std::string> generate_one_example(std::string* country = nullptr, std::string* state = nullptr);
    ...
    std::discrete_distribution<size_t>  state_distribution    { 0.1, 0.90 };
